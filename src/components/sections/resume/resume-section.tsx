            ...
            <div className="container max-w-7xl mx-auto pb-10">
                ...
                <ul ref={listParent} className="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    {filteredJobs.map((job, i) => {
                        ...
                        const jobKey = `${jobTitle} at ${company}-${i}`  // Appended index to the jobKey
                        return (
                            <li key={`${jobKey}-${Math.random()}`} className={cn(i !== 0 ? "py-12" : 'pb-12')}>
                                <article>
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                <div className="prose max-w-none text-gray-500 dark:text-gray-400">
                                                    <ul className="list-disc">
                                                        {jobNotes.map(({ note }, noteIndex) => (
                                                            <li key={`${jobKey}-${note.slice(0, 10)}-${noteIndex}`}>{note}</li>  // Appended noteIndex to the key
                                                        ))}
                                                    </ul>
                                                </div>
                                            </div>
